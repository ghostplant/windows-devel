...
namespace CAudio {
	...
	static float ceof[SAMPLENUM];

	void Initialize() {
		for (unsigned j=0;j<SAMPLENUM;j++)
			ceof[j]=(0.54-0.46*cos(acos(-1.0)*(j<<1)/(SAMPLENUM-1)))*0.000030517578125f;
	}

	float standard[BUFSIZE];

	int hillCmp(const void *a,const void *b) {
		return standard[*(int*)a] < standard[*(int*)b] ? 1:-1;
	}
	...
	void DrawFrequency(LPSTR lpData) {
		...
		static int hill[SAMPLENUM];
		int hillCnt=0;

		for (int i=1;i<PREFIX;i++)
			if (standard[i]>standard[i-1] && standard[i]>standard[i+1])
				hill[hillCnt++]=i;

		qsort(hill, hillCnt, sizeof(int), hillCmp);

		hillCnt=min(hillCnt,3);

		//while (hillCnt>1 && standard[hillCnt-1] < max(*standard*0.25, 2.0f))
		//	--hillCnt;

		static char title[5000], *at;
		static int list[500];
		at=title;

		for (int i=0;i<hillCnt;i++) {
			int value=2*hill[i]*sndfmt.nSamplesPerSec/SAMPLENUM;
			while (value<256)
				value<<=1;
			while (value>=512)
				value>>=1;
		...
		qsort(list, hillCnt, sizeof(int), intCmp);
		for (int i=0;i<hillCnt;i++) {
			sprintf(at,"%d ",list[i]);
			at+=strlen(at);
		}
		...
		for (int j=0;j<hillCnt;j++) {
			int i=hill[j];
			glVertex2f(i*2.0/PREFIX-1, -0.8f);
			glVertex2f(i*2.0/PREFIX-1, standard[i]/16.0f - 0.8f);
		}
		glEnd();
